...
PRICE_LEVEL_RULE = """- If user says "дёшево/недорого/бюджетно" => price_level = 1
...
SYSTEM_PROMPT = """Ты — компонент извлечения структурированных фильтров для поиска заведений (places).
...
def validate_and_normalize(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def norm_tags(x: Any) -> List[str]:
        ...
        return _dedupe_keep_order([t for t in x if t in set(ALLOWED_TAGS)])

    must_have = norm_tags(obj["must_have"])
    nice_to_have = norm_tags(obj["nice_to_have"])
    excluded = norm_tags(obj["excluded_tags"])

    selected = _dedupe_keep_order(must_have + nice_to_have)
...
def build_user_prompt(user_query: str) -> str:
    payload = {
        ...
        "PRICE_LEVEL_RULE": PRICE_LEVEL_RULE,
...
def extract_filters_openai(
    ...
    *,
    ...
    return validate_and_normalize(obj)
...
if __name__ == "__main__":
    ...
    result = extract_filters_openai(example_query)
