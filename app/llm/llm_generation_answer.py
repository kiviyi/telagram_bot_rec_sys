...
SYSTEM_PROMPT = """Ты — помощник, который создает естественные ответы на русском языке о рекомендованных заведениях.
...
def get_place_descriptions_from_db(place_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    """
...
def build_user_prompt(places_data: List[Dict[str, Any]]) -> str:
    """
...
def generate_answer_openai(
    ...
    *,
    ...
    """
    user_prompt = build_user_prompt(places_data)
...
def main():
    ...
    # Получаем описания из базы данных
    print("Получение описаний мест из базы данных...")
    db_descriptions = get_place_descriptions_from_db(place_ids)
    ...
    try:
        answer_result = generate_answer_openai(client, places_data, model=MODEL_NAME)
        ...
        print(f"Ответ успешно сгенерирован и сохранен в {output_file}")
